...
if check_password():
    ...
    f"""
**Current Total Invite Count:** {sum(map(int, df["PlusOnes"])) + len(df) - 1}
"""

    initials = st.text_input("Initials")
    name = st.text_input("Name of Invite")
    plus_ones = st.number_input("Plus Ones", min_value=0, max_value=2)
    ...
    if generate:
        ...
        template.paste(qr.resize((qr_width, qr_height)), (qr_x, 1170))
